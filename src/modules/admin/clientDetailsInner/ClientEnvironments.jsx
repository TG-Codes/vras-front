...
const ClientEnvironments = (props) => {
  ...
  const datasource = {
    getRows(params) {
      let url = `${baseUrL}/admin/clients/show/${paramsData?.slug}`;
      ...
      try {
        ...
        response
          .then((response) => {
            // if (response.data.data.environment.length > 0) {
              params.successCallback(
                [response.data.data.environment],
  ...
  const envColumnDefs = [
    { field: "name", width: 470 },
    { field: "description", width: 470 },
    // {
    //   field: "Action",
    //   width: 430,
    //   cellRenderer: (params) => {
...
export default connect(mapStateToProps, {
  ...
  loaderStateFalse,
  loginSuccess,
})(withNamespaces()(ClientEnvironments));
