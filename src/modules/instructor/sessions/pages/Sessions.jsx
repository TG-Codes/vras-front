...
import SessionsAddEdit from "../../../../utility/components/instructor/SessionAddEdit";
...
const Sessions = (props) => {
  ...
  const fetchLiveSessionData = async ()=> {
    try {
      const response = await axios.get(`${baseUrL}/instructors/live-sessions`, {
        headers : {
  ...
  const columnDefs = [
    ...
    { field: "sessionRecording", minWidth:150, maxWidth:200 },
    { field: "status", minWidth:150, maxWidth:200 },
    {
      field: "Action", minWidth:500, maxWidth:200,
      cellRenderer: (params) => {
        if (params.data) {
          return (
            <>
  ...
  const statusValue = ["success", "failure"];
  const formatData = () => {
    let data = {};
  ...
  const datasource = {
    getRows(params) {
      let url = `${baseUrL}/instructors/live-sessions?`;
  ...
  const onRowClick = (e, params)=> {
    ...
    navigate(`/instructor/live-session-details/${e?.data?.id}`)
    }
  }
  return (
    <>
        ...
        {true && (
          <div className="table_modal_open">
            <div className="back_btn_caption_hold">
              {location.pathname=="/instructor/sessions" &&
